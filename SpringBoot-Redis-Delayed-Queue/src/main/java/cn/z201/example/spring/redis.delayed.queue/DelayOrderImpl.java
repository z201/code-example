...
public class DelayOrderImpl implements DelayOrderI<OrderBo> {
        ...
        threadPoolTaskExecutor.execute(() -> {
            ...
            for (;;) {
                try {
                    ...
                    // 测试下直接获取所有数据。
                    Set<Integer> data = redisTemplate.opsForZSet().rangeByScore(ORDER_DELAY_KEY, 0,
                            DateTool.currentTimeMillis());
    ...
    public List<OrderBo> all() {
        ...
        orderZSetMap.forEach((key, value) -> {
            list.add(OrderBo.builder().id(Long.valueOf(key)).orderDeadlineTime(value)
