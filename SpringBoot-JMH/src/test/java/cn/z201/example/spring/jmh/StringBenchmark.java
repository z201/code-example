...
public class StringBenchmark {
    ...
    public synchronized void initialize() {
        stringList = new Random().ints().limit(size).boxed().map(Object::toString).collect(Collectors.toList());
    ...
    public void executeJmhRunner() throws RunnerException {
        Options opt = new OptionsBuilder()
                ...
                // reports
                .result("StringBenchmark.json").shouldFailOnError(true).jvmArgs("-server").build();
        new Runner(opt).run();
    }
    ...
    public void testStringJoin(Blackhole blackhole) {
        String str = "";
        stringList.forEach(String::join);
    ...
    public void testStringBuffer(Blackhole blackhole) {
        ...
        stringList.forEach(stringBuffer::append);
