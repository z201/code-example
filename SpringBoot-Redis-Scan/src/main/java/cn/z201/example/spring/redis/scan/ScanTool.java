...
public class ScanTool {
     ...
     * 用户在下次迭代时需要使用这个新游标作为 SCAN 命令的游标参数， 以此来延续之前的迭代过程。 当 SCAN 命令的游标参数被设置为 0 时，
     * 服务器将开始一次新的迭代， 而当服务器向用户返回值为 0 的游标时， 表示迭代已结束。
     * @param pattern
     * @param count
     * @return
     */
     ...
     * <p>
     * 可用版本： >= 2.8.0 时间复杂度：增量式迭代命令每次执行的复杂度为 O(1) ， 对数据集进行一次完整迭代的复杂度为 O(N) ， 其中 N
     * 为数据集中的元素数量。
     * @param key
     * @param pattern
     * @param count
    ...
    public Map<String, String> hScan(String key, String pattern, Integer count) {
        ...
        while (cursor.hasNext()) {
            ...
            if (null != data) {
                map.put(stringRedisSerializer.deserialize(data.getKey()),
                        stringRedisSerializer.deserialize(data.getValue()));
            }
        }
        return map;
    }
