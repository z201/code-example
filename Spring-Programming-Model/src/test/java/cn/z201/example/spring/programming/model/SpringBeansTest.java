...
import cn.z201.example.spring.programming.model.beans.SpringBeans;
...
public class SpringBeansTest {
    ...
    /**
     * 1. 定义 Spring Bean -> this#createBeanDefinition 2. BeanDefinition 元信息 ->
     * this#createBeanDefinitionAlias 3. 命名 Spring Bean -> this - 可以自定义Spring
     * Bean的名称，也可以使用Spring默认生成Bean名称。 - 在xml配置中自定义比较常见，在注解中大多使用默认生成。 4. Spring Bean 别名 -
     * alias 别名,通过beanName 和 beanNameAlias 获取的实例实际上是一个Bean。 5. 注册Spring Bean ->
     * IocContainerTest - DefaultListableBeanFactory 基于xml配置信息创建bean -
     * AnnotationConfigApplicationContext 基于注解类创建bean - 基于@Bean - 基于@Component - 基于@Import
     * - 外部Bean创建 6. 实例化 Spring Bean - Bean 初始化 BeanInitialization ->
     * InitBeanInitializationTest - @PostConstruct 标注 public void method() -
     * 实现InitializationBean接口 afterPropertiesSet() - 指定@Bean init-method 方法名称。 - 执行顺序不变
     * PostConstruct afterPropertiesSet() init-method -
     * AbstractBeanDefinition#setInitMethodName 8. 延迟初始化 Spring Bean - @Lazy(true or
     * false) - xml lazy-init 9. 销毁 Spring Bean ->InitBeanInitializationTest - @PreDestroy
     * - DisposableBean#destroy - 指定@Bean destroy-method 方法名称。 10. 垃圾回收 Spring Bean -
     * Object#finalize() 方法
     */
    @Test
    public void setUp() {
    ...
    public void createBeanDefinition() {
        ...
        // AnnotationBeanNameGenerator：基于注解扫描的 BeanNameGenerator 实现。在使用@Component
        // 相关派生注解扫描生成。
    }
    ...
    public void createBeanDefinitionAlias() {
        ...
        SuperInjectionBean superInjectionBeanAlias = beanFactory.getBean("superInjectionBean-alias",
                SuperInjectionBean.class);
